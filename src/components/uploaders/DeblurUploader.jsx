...
const DeblurrUploader = () => {
  ...
  const processImage = async () => {
    try {
      ...
      formData.append("base64_string", imageSrc);
      console.log(imageSrc);
      console.log("Sending image to server...");
      const response = await axios.post(
        "http://127.0.0.1:5000/denoise",
        formData,
        {
          headers: {
      ...
      console.log("Received response data: ", response.data.path);
  ...
  return (
    <div>
      ...
      <div className="image-container">
        {imageSrc && (
          <img src={imageSrc} alt="" className="photo-image-class" />
        )}
      </div>
      ...
      <button onClick={processImage} className="upload-image">
        {" "}
        Upload Image
      </button>
...
export default DeblurrUploader;
